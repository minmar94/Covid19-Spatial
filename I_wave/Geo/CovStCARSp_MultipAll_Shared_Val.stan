...
real log_RichLin(real lr, real lh, real pe, real ls, real ti) {
  ...
  real sparse_carar_lpdf(vector phi, vector phiOld, real rho, real tau, real alpha, 
...
transformed parameters {
    ...
    model {
        ...
        beta ~ normal(0, 10);
        // CAR prior
        phi[1] ~ sparse_carar(zeros, rho, tau, alpha, 
                            W_sparse, W_weight, D_sparse, lambda, Nreg, W_n);
        for (i in 2:Ntimes){
              phi[i] ~ sparse_carar(phi[i-1], rho, tau, alpha, 
                                  W_sparse, W_weight, D_sparse, lambda, Nreg, W_n);
        }
        alpha ~ beta(.5, .5);
        tau ~ gamma(2, 2);
	...
	// Out of sample function
        for (i in 1:NTest) {
            lmTest[i] =  phi[tIdTest[i]][rIdTest[i]] + log_sum_exp(logbase, log_RichLin(logr, logh, p, logs, t[tIdTest[i]]));
           }
